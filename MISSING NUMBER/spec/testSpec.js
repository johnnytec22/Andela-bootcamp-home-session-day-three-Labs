...
describe("Find missing number two lists: ", function() {
  ...
  describe("Case for lists with the same entries", function() {
    ...
    it('should return 0 for [4,2,1,3], [2,3,1,4]', function(){
      expect(myApp.findMissing([4,2,1,3], [2,3,1,4])).toEqual(0);
    })

    it('should return 0 for [30,20,10], [10,20,30]', function(){
      expect(myApp.findMissing([30,20,10], [10,20,30])).toEqual(0);
    })
  ...
  describe("Case for lists with similar entries and a missing number", function() {
    ...
    it("should return 5 for [-2, -4, -6], [-6, -2, -4, -8]", function() {
      expect(myApp.findMissing([-2, -4, -6], [-6, -2, -4, -8])).toEqual(-8);
    });
